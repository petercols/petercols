# 👋 Hola, soy Pedro Marambio

**Senior Software Engineer · Backend & Bases de Datos**

Construyo y mantengo productos web en producción, con foco en backend, modelado de datos y arquitectura de sistemas.

📍 Santiago, Chile · [LinkedIn](https://linkedin.com/in/pedromv8) · pmarambiovdev@gmail.com

---

## 🚀 Sobre mí

Ingeniero de software con más de 5 años construyendo y manteniendo productos web en producción. Me especializo en **desarrollo backend y bases de datos**: APIs REST con Node.js, modelado y optimización de bases de datos relacionales, automatización con Python y arquitectura de sistemas, con dominio del frontend para aportar como ingeniero completo cuando el proyecto lo requiere.

He sido responsable técnico único de plataformas en producción, cubriendo además DevOps, respuesta a incidentes y ciberseguridad. Destaco por traducir lo técnico a lenguaje de negocio y comunicarme con fluidez con cualquier interlocutor, desde un equipo de desarrollo hasta el cliente final.

---

## 🏢 Bokel - Consultoría en Ingeniería de Software

Fundé **[Bokel](https://bokel.cl)** en 2025 como mi consultoría en ingeniería de software. Acompaño a emprendedores y equipos a construir productos sólidos de extremo a extremo: desde el levantamiento de requerimientos hasta el despliegue en producción.

- 🔧 Desarrollo de productos web con foco en backend y bases de datos
- 🏗️ Construcción de plataformas propias de extremo a extremo
- 🤝 Acompañamiento técnico: revisión de código, decisiones de arquitectura y traducción de lo técnico a prioridades de negocio

🌐 **[bokel.cl](https://bokel.cl)**

---

## 💼 Lo que hago

- Desarrollo backend con Node.js y diseño de APIs REST
- Modelado y optimización de bases de datos relacionales
- Arquitectura de sistemas y plataformas escalables
- Automatización y scraping con Python
- DevOps, respuesta a incidentes y hardening de servidores
- Acompañamiento técnico para emprendedores y startups

---

## 🛠️ Stack Técnico

**Lenguajes**
JavaScript/TypeScript (ES6+), Python, SQL · también PHP

**Backend**
Node.js, Express, REST APIs · también Laravel, CodeIgniter

**Bases de Datos**
PostgreSQL, MySQL, MongoDB, Prisma, Sequelize

**DevOps & Cloud**
Linux (Ubuntu), Docker, Google Cloud Platform, Grafana

**Frontend**
React.js, HTML5, CSS3, Tailwind CSS

**Herramientas**
Git, GitHub, Jira, Selenium, herramientas de IA (Claude Code, Cursor)

---

## 🎯 Proyectos Destacados

- **[QSales](https://qsales.cl/)** — Plataforma de integración B2B para el sector farmacéutico que extrae, consolida y procesa información diaria desde portales de retail (sell out, quiebres, stock y geolocalización de locales). Reconstruí el sistema completo desde cero: rediseñé y refactoricé una plataforma legacy lenta y obsoleta, migrándola a Node.js, React y API REST con autenticación JWT. La nueva versión quedó en producción reemplazando al sistema anterior, sirviendo a 40 clientes corporativos (GSK, Dentaid, FAES). Optimización de algoritmos críticos que redujo cargas de dashboards de 60s a 3-5s.
- **Latigou Marketplace** - Arquitectura completa, modelado de base de datos y especificación de APIs para plataforma de turismo.
- **Pases** - Plataforma boutique de venta de tickets para eventos; arquitectura, base de datos e integración de pagos.

---

## 🧗 Fuera del código

Cuando no estoy frente a una terminal, probablemente esté escalando un boulder, en un cerro de la cordillera, o leyendo sobre lo que pasa a años luz de aquí. La ciberseguridad y el ethical hacking me obsesionan tanto como el death metal a buen volumen mientras programo. Creo que la disciplina del entrenamiento físico y la del software se parecen más de lo que la gente piensa: ambas premian la constancia y castigan los atajos.

---

## 📫 Contacto

- ✉️ **Email:** pmarambiovdev@gmail.com
- 💼 **LinkedIn:** [linkedin.com/in/pedromv8](https://linkedin.com/in/pedromv8)
- 🌐 **Bokel:** [bokel.cl](https://bokel.cl)

> Disponible para proyectos backend en modalidad part-time / freelance.
